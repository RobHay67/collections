...
def set_comic_series(scope, widget_key):
	# comic_file is the master list of comics
	selected_series = (scope[widget_key])

	scope.comic_selected_series = selected_series
	
	# this currently defaults to vol 1
	# scope.comic_selected_volume = 
...
def create_comic_df(scope):
	...
	comic_df = scope.comic_file
	
	selected_series = scope.comic_selected_series
...
def series_selector(scope):
	...
	previous_selection = scope.comic_selected_series
